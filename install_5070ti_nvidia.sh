...
preflight_checks() {
    ...
    if ! lspci 2>/dev/null | grep -qi nvidia; then
        die "No NVIDIA GPU detected via lspci. Is the GPU seated correctly?"
    fi
    local gpu_info
    gpu_info=$(lspci 2>/dev/null | grep -i nvidia | head -n1)
    success "NVIDIA GPU detected: ${gpu_info}"

    # Confirm the GPU is Turing or newer (open modules only support Turing+)
    if ! lspci 2>/dev/null | grep -qiE \
        "RTX (50[0-9]{2}|40[0-9]{2}|30[0-9]{2}|20[0-9]{2})|GTX 16[0-9]{2}"; then
        warn "Could not confirm a Turing+ GPU from lspci output."
        warn "Detected PCI line: ${gpu_info}"
        warn "The open kernel modules require Turing (RTX 20xx / GTX 16xx) or newer."
        warn "Proceeding — verify your GPU is supported before rebooting."
    fi
    ...
    for cmd in git xbps-install xbps-remove xbps-query xbps-reconfigure lspci dkms; do
